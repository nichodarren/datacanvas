...
| **R-16** | **Seluruh project hanya ada di satu laptop** — tanpa remote, tanpa salinan di luar mesin | Eksekusi | Sedang | **Kritis** | Belum ada. Ditunda secara sadar (2026-07-29); `scripts/check.sh` membuat pindah host nanti murah, tapi itu **bukan** mitigasi untuk kehilangan data | Tidak ada sinyal dini — kegagalan disk tidak memberi peringatan. Itulah yang membuat risiko ini berbeda dari yang lain di tabel ini |
| **R-17** | **CI belum pernah dieksekusi sekali pun** — perilaku di Linux tak terverifikasi | Teknis | **Tinggi** | Sedang | `scripts/check.sh` menjalankan seluruh rangkaian di runner mana pun; `.gitattributes` memaku line ending; audit manual atas asumsi Windows sudah dilakukan (tidak ada path hardcoded, tidak ada tabrakan case) | **Sudah terjadi sekali:** review Gerbang 1 menemukan aplikasi tidak jalan di bawah uvicorn — cacat khusus Windows yang lolos 236 test. Kebalikannya (khusus Linux) tidak akan ketahuan sampai deploy |
