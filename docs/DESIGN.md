...
| **Versi** | 0.14.1 |
| **Status** | 🟢 **Baseline aktif** — seluruh keputusan D-001…D-037 Accepted; Gerbang 0, 1 & 2 terlampaui; dokumen ini mengikat untuk implementasi |
| **Tanggal** | 2026-08-01 |
...
| 0.10.0 | 2026-07-31 | **Perubahan scope UI (§0.3 aturan 3) — pemilik produk memangkas permukaan Fase 2 setelah memakainya sendiri.** Justifikasinya konsisten di semua item: *produk meminta terlalu banyak keputusan kecil dari orang yang baru datang untuk melihat datanya*, dan menghabiskan ruang layar untuk hal yang belum bisa dipakai. Yang berubah di dokumen ini: **(1) FR-D.1 dipecah** — `FR-D.1a` (paging sisi server, P0, **terpenuhi & terukur**) dan `FR-D.1b` (menjelajah seluruh dataset, **ditunda**). Grid tervirtualisasi diganti pratinjau 10 baris + tombol *View more* (+50/klik). Pemecahannya bukan formalitas: pada 50 baris per klik, baris ke-5.000.000 berjarak seratus ribu klik — pola itu **secara struktural** tidak bisa memenuhi 'jutaan baris', dan mempertahankan rumusan lama akan membuat dokumen ini berbohong. Menghapusnya utuh juga salah, karena kemampuannya nyata dan terukur (78 ms/halaman, 69 ms melompat ke baris 2.500.000). **(2) FR-D.2 terpenuhi** oleh tombol itu — requirement-nya tidak menentukan bentuk kontrolnya. **(3) Indikator null% dicabut dari FR-D.4**; header kolom kembali menjadi label (nama + tipe), bukan dasbor kualitas. Null% tidak pernah dibangun, slotnya sempat diisi confidence yang juga dicabut. **(4) FR-C.6 DICABUT** (bukan ditunda — permukaan yang benar sudah ada: `type_consistency_report` P0 dan kartu tab Profile) — seluruh peringatan deteksi (pil `check`, lencana `✓ set`, banner, tab Schema, badge kartu) dihapus; `detection_confidence`/`detection_reason` tetap dihitung dan tersimpan, jadi ini pencabutan tampilan, bukan fakta. **(5) Baris keputusan §14.2 'versi di header, selalu' dicabut** — beserta sidebar navigasi, panel Run Log, dan composer copilot, yang bersama-sama menghabiskan ± sepertiga lebar layar 1280px untuk ruang kosong. **Prinsip P4 tidak disentuh**: INV-2, INV-3, dan trigger Postgres yang menegakkannya tetap, karena §9.4 menyusun fingerprint dari `dataset_version_id` + `schema_contract_id` dan INV-6 bergantung padanya. **(6) Catatan Gerbang 2 diperbarui** — tetap ditutup, tapi kini berdiri di atas FR-D.1a saja; tiga baris pengukurannya mengukur jalur browser yang sudah diganti. Ditambah dua cacat yang ditemukan riset, keduanya buatan sendiri: `AccountMenu` mendeklarasikan `role="menu"` tanpa satu pun perilaku keyboard yang dijanjikan peran itu (diganti pola disclosure), dan *Sign out on all devices* tayang tanpa konfirmasi — **melanggar NFR-UX.1/UX-7**, aturan kita sendiri. FR-A.2 juga akhirnya punya permukaan: halaman akun dengan daftar sesi aktif dan pencabutan per-perangkat (OWASP Session Management), plus ganti password yang mencabut sesi lain dan mematikan token reset yang beredar. **Tabrakan yang dicatat tapi belum diselesaikan:** FR-D.5 memesan gestur klik header untuk profil kolom, dan tabrakan itu **diselesaikan di sesi yang sama**: profil pindah ke tab Profile tersendiri (kartu ringkas per kolom → klik → univariat lengkap), sehingga klik header tetap milik FR-C.2. Dicatat bersama tiga syaratnya — profil ringkas vs lengkap adalah dua hal berbeda (NFR-PERF.2), INV-5 menempatkan tab ini di Fase 3 karena kartunya penuh angka yang butuh `computation_id`, dan badge tipe memakai kosakata logis §9.2, bukan tipe fisik. |
| 0.11.0 | 2026-07-31 | **FR-D.3 terpenuhi — kontrol kolom, dan scroll horizontal kembali secara bersyarat.** Requirement P1 ini lama terbaca sebagai kenyamanan; keputusan pratinjau tanpa-scroll (0.10.0) menaikkannya jadi keharusan, karena **ingest tidak punya batas jumlah kolom** dan 1230px dibagi 60 tidak terbaca oleh siapa pun. Dibangun: **sembunyikan** (centang di panel `Columns`, kolom terakhir ditolak), **pin** (toggle — bukan centang kedua, karena dua centang sebaris terbaca sebagai dua bagian dari satu pilihan; kolom yang di-pin tetap pada urutan file), dan **perlebar** (tarik tepi kanan header; lebar hanya disimpan untuk kolom yang benar-benar ditarik). **Scroll horizontal dikembalikan bersyarat** — tanpa itu `pin` dan `perlebar` adalah kontrol yang tidak melakukan apa-apa. Aturannya: lebar minimum 96px per kolom; selama muat tidak ada scrollbar sama sekali. **96 bukan angka bulat** — draf pertama memakai 110 yang justru memunculkan scrollbar di file 12 kolom, persis desain yang baru disetujui; batas atasnya 98. Ditambah default 15 kolom untuk file lebar, supaya tidak ada yang bertemu dindingnya sebelum menemukan kontrolnya. **`Dipindah urutannya` dicabut** — dibangun, dipakai, lalu dihapus: 120 tombol panah di daftar 60 kolom demi mengubah urutan yang sudah dipilih pembuat file. **FR-C.6 dicabut penuh** (0.10.0 sempat menulisnya 'ditunda'): permukaan yang benar sudah ada — `type_consistency_report` (tool P0) dan kartu tab Profile — dan seluruh rujukannya dialihkan, bukan ditinggal menggantung. Dua cacat lain diperbaiki di sepanjang jalan, keduanya berpola sama: sebuah aturan diletakkan menempel pada satu benda alih-alih menyebut semua yang seharusnya tercakup. Scrollbar putih di panel `Columns` — akarnya halaman tidak pernah memberi tahu browser bahwa temanya gelap (`color-scheme: dark`), dan aturan scrollbar hanya ditulis untuk `.grid-wrap`. Keduanya kini berlaku untuk setiap permukaan yang menggulir. |
| 0.12.0 | 2026-07-31 | **Penutup sesi — dua keputusan sesi ini diangkat jadi ADR.** Keduanya sudah tercermin di FR dan changelog, tapi belum punya catatan keputusan sendiri, sehingga konteks & alternatif yang ditolaknya akan hilang. **D-031 · Akun punya halamannya sendiri; dropdown hanya peluncur.** FR-A.2 (P0) terimplementasi penuh di backend sejak Fase 1 lalu **tidak pernah diberi tombol** — satu-satunya cara keluar adalah menghapus cookie lewat DevTools, sementara setiap test per-lapis hijau karena lubangnya berada **di antara** lapisan. Ditambah daftar sesi + pencabutan per-perangkat (OWASP Session Management; seluruh kolomnya sudah ada di `user_session`, nol migrasi) dan ganti password yang mencabut sesi lain serta mematikan token reset yang beredar. Mencatat pula tiga cacat buatan sendiri pada dropdown versi pertama: `role="menu"` tanpa satu pun perilaku keyboard yang dijanjikan peran itu, panel yang tidak bisa ditutup dari keyboard, dan aksi *sign out everywhere* tanpa konfirmasi — **melanggar NFR-UX.1/UX-7**, aturan kita sendiri. **D-032 · Permukaan kosong dicabut, bukan disimpan sebagai kerangka.** Sidebar, Run Log, composer, dan indikator privasi dihapus; yang menjatuhkannya angka, bukan selera — ± sepertiga lebar layar 1280px (NFR-UX.4) untuk ruang yang tidak bisa disentuh, mendesak grid pratinjau yang justru satu-satunya permukaan bekerja. Dicatat juga bahwa argumen "ajarkan tata letak sekali" berlaku untuk **region**, bukan kontrol, dan bahwa pil privasi berada di kelas berbeda karena ia **menyatakan keadaan**, bukan mengumumkan ketiadaan. Tidak ada perubahan scope baru di entri ini — seluruhnya sudah dijustifikasi di 0.10.0 dan 0.11.0; ini melengkapi §18 agar keputusannya tidak diperdebatkan ulang tanpa konteks. |
| 0.13.0 | 2026-07-31 | **Sesi kualitas UI — satu cacat kebenaran data, dan alat yang seharusnya menangkapnya** (§0.3 aturan 2 → minor version). **Tidak ada perubahan scope: nol requirement ditambah, dicabut, atau dilonggarkan** (§0.3 aturan 3 tidak terpicu). FR-B.2 sempat diusulkan dicabut lalu **dibatalkan oleh pemilik produk sebelum dokumen disentuh** — ia berdiri utuh, dan konsekuensi yang membuat pembatalan itu penting sudah tercatat: begitu sebuah Dataset bisa punya dua versi, P4 berhenti menjadi kosmetik, sehingga FR-B.2 dan kembalinya identitas versi adalah **satu pekerjaan**, dijadwalkan sesi berikutnya. **Cacat utamanya:** `PreviewGrid` mengambil sel dengan indeks kolom *yang terlihat* sementara rute baris mengirim satu sel per kolom *di file* — sembunyikan kolom mana pun selain yang terakhir dan setiap kolom di kanannya menampilkan nilai tetangganya, tanpa error dan tanpa sel kosong. Bertipe benar di kedua sisi, jadi `tsc` puas dan build sukses; dan ia hidup persis di dalam `hide`, bagian FR-D.3 yang 0.11.0 naikkan dari kenyamanan jadi keharusan untuk file lebar. Diperbaiki secara struktural — sel dicari lewat **nama**, memakai `RowPage.columns` yang selama ini dibuang — sehingga mode gagalnya lenyap alih-alih ditambal. **D-033** memasang test perilaku frontend (vitest + Testing Library), tanpanya cacat itu tak mungkin ditangkap siapa pun; test-nya ditulis merah lebih dulu. **D-034** memberi unggahan indikator progres (NFR-UX.2 menuntutnya di atas 500 ms; unggahan 480 MB berjalan **28 detik** dengan label statis), sekaligus menetapkan bahwa indikatornya **berhenti mengklaim angka** begitu byte terakhir terkirim — sisanya normalisasi + inferensi seluruh file yang tidak teramati dari browser. **NFR-UX.3 ternyata tidak terpenuhi, dan `globals.css` mengklaim sebaliknya:** `--ink-faint` terukur 4,1:1 di `--bg`, 3,8:1 di `--panel`, dan **3,4:1** di `--panel-2`, terhadap ambang AA 4,5:1 — dipakai untuk kata `null` di sel grid, nomor baris, dan setiap hitungan di layar pertama. Dinaikkan ke `#8290a6`, dan kini **ditegakkan test yang mengukur rasio, bukan memaku hex**, sehingga palet tetap bebas bergerak selama tetap terbaca. **§14.5 mendapat aturan baru untuk error state**, ditulis dari layar sungguhan: saat `/auth/me` menjawab 500, beranda tetap merender "Start by adding data" beserta tiga langkahnya — tanpa drop zone, tanpa sampel, tanpa menu akun, karena semuanya bergantung pada identitas yang baru saja gagal diambil. Ia menyuruh menambah data lalu menghapus setiap caranya, dan menampilkan `500 Internal Server Error` sebagai teks untuk pengguna. Aturan §14.5 yang melarang itu **sudah ada sejak 2026-07-29** — ditulis untuk empty state, tidak pernah diuji terhadap error state. **Tiga popup ternyata mengimplementasikan tiga tingkat kelengkapan berbeda** dari satu kontrak yang sama; popup tipe tidak punya cara ditutup sama sekali. Disatukan ke `useDisclosure`. **FR-D.3 kini punya jalur keyboard dan sentuh:** `Shift`+`←`/`→` pada header yang fokus, dipilih di atas gagang tarik yang bisa difokus karena yang terakhir menambah **enam puluh tab stop** di file enam puluh kolom — aritmetika yang sama yang mencabut tombol urut di 0.11.0. Ditambah `dataset_name` pada respons versi: halaman versi tidak pernah menyebut dataset mana yang sedang dibuka, efek samping tak disengaja dari pencabutan judul di 0.10.0. Dan pilihan kolom kini bertahan antar-kunjungan, per versi. |
| 0.14.0 | 2026-08-01 | **Panduan pihak ketiga masuk, dengan hierarki yang dinyatakan lebih dulu** (§0.3 aturan 2 → minor version; **nol perubahan scope**). **D-035** mem-vendor tiga skill Vercel — `react-best-practices`, `composition-patterns`, `web-design-guidelines` — ke `config/skills/` **di dalam repo**, karena skill adalah instruksi yang membentuk cara kerja di sini, dan instruksi yang berbeda antar mesin serta berubah tanpa jejak adalah persis yang project ini tolak untuk semua hal lain yang mempengaruhi hasil. **Aturan presedensinya yang paling penting: DESIGN.md mengalahkan setiap skill; yang meyakinkan kita sebaliknya menjadi ADR, bukan suntingan diam-diam.** Tanpa itu, panduan generik akan menggerus posisi yang dibayar mahal — hampir semuanya mendorong component library, hierarki visual lebih ramai, dan motion, sementara §10.3 menolak component library, P9 memilih permukaan kecil, dan D-032 baru mencabut sepertiga lebar layar. Dua konflik sudah muncul, keduanya gaya rumah, keduanya diputus untuk DESIGN.md: **Title Case** (kita sentence case) dan **curly quotes** (kita lurus). **Dua dari lima paket sengaja tidak dipasang, dan alasannya dicatat supaya tidak dievaluasi ulang dari nama repo saja:** `frontend-design` berkerangka identitas visual (hero sebagai tesis, pasangan typeface, *"ambil satu risiko estetis"*) yang bukan alat analisis internal — sepertiga terakhirnya **dipanen** ke §14.4; UI/UX Pro Max ternyata **tujuh skill, 23 MB**, lima untuk logo/banner/slide/brand, dengan **33 skrip dapat dieksekusi** termasuk `shadcn_add.py` yang memasang persis component library yang §10.3 tolak — checklist UX-nya, satu-satunya bagian yang relevan, tumpang tindih dengan daftar Vercel yang 4 KB dan tanpa skrip. **§14.4 bertambah dua tabel hasil panen:** W-1…W-6 (kata-kata di antarmuka adalah bahan desain — namai lewat apa yang dikendalikan pengguna; kontrol menyebut apa yang terjadi; nama aksi konsisten sepanjang alur; error tidak minta maaf) dan Q-1…Q-5 (lantai kualitas). W-4 tiba di tempat yang sama dengan §14.5, lewat jalan berbeda — §14.5 lahir dari layar 500 yang rusak, W-4 dari panduan yang belum pernah membacanya. **`web-design-guidelines` dimodifikasi secara lokal** agar membaca aturan ter-commit alih-alih mengambilnya dari GitHub tiap review: kumpulan aturan yang berubah antar-run bukan kumpulan aturan. **Dan alatnya langsung membuktikan dirinya pada hari pertama, dua kali, di kode berumur beberapa jam:** `gridPreferences.ts` menulis `localStorage` tanpa versi skema (`client-localstorage-schema`) — diperbaiki di sini beserta test-nya; dan `📌` dipakai sebagai ikon tombol pin — dicatat **Q-4**, menunggu keputusan visual, bukan diganti sepihak. |
| 0.14.1 | 2026-08-01 | **`frontend-design` dipasang, membalik inventaris D-035** (§0.3 aturan 1 — kebijakannya tidak berubah, hanya daftar apa yang terpasang; nol perubahan scope). Pemicunya jawaban pemilik produk atas *"apa yang terasa belum matang?"* — **"kurang punya karakter dan minim dipertimbangkan"** — yang setelah diperiksa terhadap §18 ternyata **harfiah benar**: setiap keputusan desain yang pernah tercatat adalah pengurangan, dan **tidak ada satu pun keputusan positif tentang rupa**. Alasan penolakan semula (*"kerangkanya identitas visual"*) memangkas terlalu banyak: kerangkanya memang tidak cocok, tapi **metodenya** — susun token ringkas lalu uji terhadap *"apakah ini yang akan kuhasilkan untuk brief serupa mana pun?"* — justru menjawab persis celah itu. Amandemen ditulis **di dalam D-035**, bukan menggantikannya. |
| 0.15.0 | 2026-08-01 | **Keputusan positif pertama tentang navigasi** (§0.3 aturan 2 → minor; **nol perubahan scope**). **D-036** menaruh jejak `Project › Dataset` di slot header yang sudah ada — nol lebar tambahan, nol tinggi tambahan — sehingga argumen ongkos ruang yang mencabut sidebar (D-032) tidak tersentuh. **Hitungan D-032 diperiksa ulang dan masih benar: dua tujuan kerja**, jadi usulan mengembalikan sidebar **ditolak secara eksplisit** meski brief sesi ini membuka pintunya. Yang ditemukan sepanjang jalan lebih berat daripada keluhan yang memulainya: halaman versi tak punya apa pun yang menyebut project-nya, dan tautan brand mengembalikan pengguna ke project mana pun yang terakhir diingat `localStorage` — **membuka versi milik project lain lalu menekan brand memindahkan orang ke project berbeda tanpa satu pun tanda di layar.** Tak bisa diperbaiki dari klien: respons versi membawa `dataset_id` dan `dataset_name` dan tidak apa pun tentang project, jadi ia kini membawa `project_id` + `project_name` (**nol migrasi** — foreign key-nya sudah ada). **§14.5 bertambah baris 404** dan **alasan penolakan galeri project diperluas**: keputusannya tetap, tapi alasan lamanya hanya menyentuh pengguna baru sementara yang benar adalah ongkos klik, yang berlaku untuk pengguna kembali juga — dengan batasnya dinyatakan (belum diuji pada tiga puluh project, ditinjau Gerbang 4). Diperiksa dan dicatat: **"nol project" adalah keadaan yang tidak bisa dicapai**, karena §15.2 membuat satu otomatis dan tak ada rute menghapusnya. Ditambah `app/not-found.tsx` — 404 masih bawaan Next, berlatar terang tanpa satu pun tautan keluar, melanggar aturan §14.5 yang lahir Juli dari layar 500 berbentuk sama. |
| 0.16.0 | 2026-08-01 | **Keputusan positif pertama tentang rupa** (§0.3 aturan 2 → minor; **nol perubahan scope**). **D-037** menghapus `--accent` seluruhnya: ia sedang mengerjakan lima pekerjaan — tautan, tipe logis, tombol primer, setiap border hover, cincin fokus — dan **warna yang menandai segala yang bisa diklik tidak menyisakan apa pun untuk dikatakan ketika sesuatu benar-benar salah**. Anggaran itu disimpan untuk Fase 3, tempat `stale` dan `cached` (UX-4, UX-5) akan membutuhkannya. Penekanan kini dibawa **pembalikan** — tombol primer, tipe terpilih, kolom ter-pin dan tanah terpilih semuanya memakai gerakan yang sama, jadi ia dipelajari sekali. **`--ok` ikut dicabut:** sukses adalah hasil normal, bukan pengecualian, dan *"Password changed."* sudah menyatakan segalanya. **Dua tanah lahir dari satu deklarasi per token** lewat `light-dark()` — bukan dua blok `:root`, karena dua blok yang harus sepakat adalah dua blok yang suatu hari tidak lagi sepakat. **Yang paling layak dicatat: menulis test lebih dulu melahirkan sebuah token.** Menahan satu warna border pada 3:1 (SC 1.4.11) akan menggambar ribuan pemisah baris grid dengan bobot outline tombol, jadi border pecah jadi tiga — `--edge` (ditahan pada 3:1 dan diuji), `--line`, `--line-soft`. **Q-2, Q-4 dan Q-5 ditutup** di §14.4, dan **premis Q-5 dikoreksi**: ambang AA adalah **24×24 (SC 2.5.8)**, bukan 44×44 — 44 adalah SC 2.5.5 (**AAA**), dan tombol kita sudah lulus AA sebelum sesi ini. `test_design_tokens.py` **13 → 48 test**, kini mengukur dua tanah, kontras non-teks, dan ketiadaan aksen. **Ongkosnya dinyatakan:** netral dingin adalah seragam hampir setiap alat teknis, jadi nol karakter boleh datang dari palet dan seluruh bebannya pindah ke bobot, tiga tier garis, dan ritme. |
...
| ~~**FR-C.6**~~ | ~~Sistem menampilkan peringatan ketika deteksi tipe berisiko~~ | **Dicabut 2026-07-31** |
| **FR-C.7** | Pengguna dapat menetapkan format khusus (format tanggal, desimal separator, penanda null seperti `-`, `N/A`, `9999`) | P1 |

> **FR-C.6 dicabut (2026-07-31), bukan ditunda.** Sempat dibangun penuh di Fase 2 — pil `check` pada header berconfidence rendah, lencana `✓ set` pada kolom yang sudah dikoreksi, banner *"N columns worth checking"*, tab Schema dengan kolom Confidence dan Why, serta badge `N to check` di kartu dataset. Dipakai sendiri oleh pemilik produk, lalu dicabut seluruhnya.
>
> Alasannya dua, dan yang kedua yang membuatnya **dicabut alih-alih ditunda**:
>
> 1. **Ia menagih di muka.** Produk meminta selusin penilaian kecil dari orang yang baru datang untuk *melihat* datanya — sebelum mereka membaca satu baris pun.
> 2. **Tempatnya salah, dan tempat yang benar sudah ada.** Header kolom adalah label, bukan dasbor kualitas. Pertanyaan *"kolom mana yang mencurigakan"* dijawab lebih baik oleh **kartu di tab Profile** (FR-D.5), yang memang permukaan untuk membaca kualitas dan yang **membungkus** untuk data lebar. Requirement ini tidak menunggu waktu yang tepat; ia menduplikasi permukaan lain dengan bentuk yang lebih buruk.
>
> **Yang tidak hilang:** `detection_confidence` dan `detection_reason` tetap dihitung dan tetap tersimpan di setiap SchemaContract (D-029), dan `detection_reason` masih muncul di popup saat mengganti tipe — satu-satunya saat ia menolong. Ini pencabutan **tampilan, bukan fakta**; kalau kartu Profile nanti ingin memakainya, datanya sudah ada tanpa migrasi maupun inferensi ulang.
>
> Nomor FR-C.6 tidak dipakai ulang. Barisnya ditahan tercoret supaya penomoran FR-C.7 tidak bergeser dan riwayatnya tetap terbaca.
...
| **FR-D.3** | Kolom dapat **disembunyikan**, **dipin**, dan **diperlebar**. ~~Dipindah urutannya~~ dicabut | P1 · **terpenuhi 2026-07-31** |
...
> **Rationale FR-D.3 — dinaikkan dari kenyamanan menjadi keharusan (2026-07-31).** Requirement ini P1 dan lama tidak dibangun karena terbaca sebagai kenyamanan. Keputusan pratinjau tanpa-scroll mengubah bobotnya: **ingest tidak punya batas jumlah kolom** — tidak ada di NFR-SCALE maupun di mana pun — sementara area kerja hanya menyisakan 1230px di layar 1280px yang dijamin NFR-UX.4. Dibagi rata itu 98px per kolom pada 12 kolom, 40px pada 30, dan tidak terbaca pada 60. Tanpa `sembunyikan`, file lebar **buntu**: bukan kurang nyaman, tapi tidak ada jalan keluar sama sekali.
>
> Tiga bagian dibangun, dan ketiganya saling bergantung:
>
> | Bagian | Bentuk | Catatan |
> |---|---|---|
> | **Sembunyikan** | Centang di panel `Columns` | Yang menutup lubangnya. Kolom terakhir tidak boleh disembunyikan — grid kosong bukan tampilan apa pun, dan kontrol pembatalnya berada di atas data yang sudah tidak ada |
> | **Pin** | **Toggle**, bukan centang kedua | Dua centang dalam satu baris terbaca sebagai dua bagian dari satu pilihan; ini dua hal berbeda. Kolom yang di-pin **tetap pada urutan file**, tidak melompat ke depan — pin adalah janji "jangan hilang dari pandangan", bukan perintah memindahkan. Menyembunyikan kolom yang di-pin melepas pin-nya |
> | **Perlebar** | Tarik tepi kanan sel header · `Shift`+`←`/`→` saat header fokus | Lebar hanya disimpan untuk kolom yang benar-benar ditarik; sisanya tetap berbagi ruang rata, dan itulah yang membuat file 12 kolom memenuhi halaman alih-alih berdesakan di kiri |
>
> **Scroll horizontal dikembalikan secara bersyarat**, dan tanpa itu dua bagian terakhir mustahil: `pin` membekukan kolom **selagi sesuatu bergulir**, dan `perlebar` butuh tempat bagi piksel tambahannya. Aturannya satu baris — tiap kolom mendapat lebar minimum 96px; selama totalnya muat tidak ada scrollbar sama sekali, lewat itu tabelnya melimpah dan menggulir. **96 bukan angka bulat**: batas atasnya 98 (1230 dikurangi 48px kolom nomor baris, dibagi 12), dan draf pertama memakai 110 yang justru akan memunculkan scrollbar di file 12 kolom — persis desain yang baru disetujui.
>
> **Di luar itu, file > 15 kolom membuka dengan sisanya tersembunyi**, supaya tidak ada yang bertemu dindingnya lebih dulu lalu baru menemukan kontrolnya.
>
> **Perlebar mendapat jalur keyboard dan sentuh (2026-07-31).** Semula ia `onMouseDown` pada `<span role="presentation">` — tak terjangkau papan ketik, tak terjangkau jari. Kini pointer event (satu jalur untuk tetikus dan sentuh, `touch-action: none` supaya browser tidak merebut gesturnya untuk menggulir), plus **`Shift`+`←`/`→` saat sel header sedang fokus**.
>
> **Gesturnya sengaja tidak diletakkan pada gagangnya.** Bentuk yang lazim menjadikan gagang tarik itu `<button>` sungguhan — lebih mudah ditemukan, dan itulah yang dilakukan kebanyakan grid. Di file 60 kolom itu **enam puluh tab stop tambahan**, masing-masing berdiri antara seseorang dan nama kolom berikutnya, demi menyetel lebar. Aritmetikanya persis yang mencabut tombol urut di atas. Sel header sudah bisa difokus karena ia tombol pengubah tipe, jadi menumpangkan gesturnya di sana berbiaya **nol** tab stop baru.
>
> **`Shift`, bukan `Alt`:** `Alt`+`←` adalah Back di Chrome dan Firefox pada Windows. `preventDefault` memang menahannya, tapi membajak pintasan navigasi browser demi lebar kolom bukan pertukaran yang layak diambil layar ini. Tidak ada tombol reset terpisah — `resize` menjepit di `MIN_COLUMN_WIDTH`, jadi menahan `Shift`+`←` **adalah** reset-nya, dan satu binding lebih sedikit untuk diumumkan. Gesturnya diiklankan lewat `aria-keyshortcuts` dan `title`; ⚠️ **diakui: ia tetap jauh lebih sulit ditemukan daripada gagang yang terlihat.** Itu harga yang dibayar untuk tidak merusak navigasi papan ketik di file lebar.
>
> ⚠️ **`Dipindah urutannya` dicabut, bukan tertinggal.** Sudah dibangun (tombol `↑`/`↓` per baris), dipakai, lalu dihapus atas keputusan pemilik produk: di file 60 kolom itu **120 kontrol** di sebuah daftar yang tugasnya membiarkan seseorang mencentang lima kotak — demi mengubah urutan yang sudah dipilih dengan sengaja oleh pembuat file. Dicatat begini supaya tiga bulan lagi tidak ada yang mengira ini pekerjaan yang belum sempat dikerjakan.
...
>
> **Indikator null% dicabut dari FR-D.4 (2026-07-31).** Keputusan pemilik produk: header kolom memuat nama dan tipe, tidak lebih. Rumusan lama menuntut null% dan **tidak pernah dibangun** — slotnya sempat diisi indikator confidence, yang ikut dicabut bersama FR-C.6. Kualitas data tidak hilang dari produk; ia pindah ke tempat yang memang dirancang untuknya: profil kolom (FR-D.5) dan peringatan PQ di §11.4. Yang berubah adalah **header kolom berhenti menjadi dasbor kualitas** dan kembali menjadi label.
>
...
| NFR-UX.3 | Kontras warna memenuhi WCAG AA; chart tetap terbaca tanpa membedakan warna saja. **Ditegakkan `tests/test_design_tokens.py`** — setiap token teks diukur terhadap setiap token latar |
...
> **NFR-UX.3 gagal dalam diam selama dua sesi, dan file-nya mengklaim sebaliknya (2026-07-31).** `globals.css` dibuka dengan komentar *"Contrast targets WCAG AA (NFR-UX.3)"*. Tidak ada yang mengukurnya, dan klaimnya keliru: `--ink-faint` terukur **4,1:1** di `--bg`, **3,8:1** di `--panel`, dan **3,4:1** di `--panel-2` — semuanya di bawah lantai AA 4,5:1 untuk teks di bawah 24px.
>
> Ia bukan token dekoratif. Ia membawa kata `null` di dalam sel grid — sebuah **fakta tentang data** — ditambah nomor baris, jumlah baris & kolom, deskripsi dataset contoh, dan petunjuk format di zona unggah.
>
> **Test-nya mengukur rasio, bukan memaku hex.** Test yang dipaku ke `#8290a6` harus disunting oleh siapa pun yang berikutnya mengubah palet — dan menyunting test agar sebuah perubahan lolos adalah cara jaminan itu menghilang diam-diam. Terikat pada rasio, palet bebas bergerak dan test hanya gagal ketika ia bergerak ke tempat yang tidak terbaca.
>
> Konsekuensi yang diterima: lantai token ini kini ditetapkan aksesibilitas, bukan selera, sehingga `--ink-faint` dan `--ink-dim` duduk lebih berdekatan daripada sebelumnya.
...
| `type_consistency_report` | "Ada tipe yang mencurigakan?" | P0 | **Pemikul utama sinyal ini** setelah FR-C.6 dicabut: pertanyaannya dijawab tool yang bisa dirujuk, bukan lencana di header |
...
| **PQ-8** | text | > 90% nilai dapat di-parse jadi tanggal | Ubah tipe → `date` (muncul lewat `type_consistency_report`) |
...
> **Header membawa jejak `Project › Dataset` di setiap layar (2026-08-01, D-036).** Ia menempati slot yang sudah ada di antara brand dan spacer — **nol lebar tambahan, nol tinggi tambahan**, jadi ongkos ruang yang mencabut sidebar tidak berlaku padanya. Nama project adalah **tautan** ke daftar dataset-nya bila kita tidak sedang di sana, dan caret di sebelahnya adalah picker; keduanya kontrol terpisah supaya jalan keluar berharga satu klik, bukan dua. Segmen versi **belum ada** dan menunggu FR-B.2 + P4.
>
> Yang membuatnya lebih dari kenyamanan: sebelum ini halaman versi tak punya apa pun yang menyebut project-nya, dan tautan brand mengembalikan pengguna ke project mana pun yang terakhir diingat browser — bisa project yang **berbeda**, tanpa satu pun tanda. Karena itu `DatasetVersionResponse` kini membawa `project_id` dan `project_name`.
...
| 4 | Halaman Preview terbuka | Pratinjau 10 baris, setiap kolom terlihat, header memuat nama + tipe logis | FR-D.1a, FR-D.2, FR-D.4 |
| 5 | Buka tab Profile | Kartu per kolom: null%, distinct, nilai teratas. `tanggal_order` terbaca `text` padahal 95% berbentuk tanggal | FR-D.5 |
| 6 | Ubah ke `date`, atur format | **SchemaContract v2**. Peringatan: 2 step sebelumnya jadi stale | FR-C.2, C.3, C.4 |
| 7 | Klik kartu `amount` | Univariat lengkap: distribusi, kuartil, 12% null, 3 outlier ekstrem | FR-D.5, FR-E.5 |
...
#### Kata-kata di antarmuka adalah bahan desain (dipanen 2026-08-01, D-035)

Diadopsi dari skill `frontend-design` the vendor, yang **tidak dipasang** — kerangkanya
untuk pekerjaan identitas visual — tapi sepertiga terakhirnya berlaku langsung di sini.
Alasan mengangkatnya ke dokumen alih-alih memasang skill-nya: aturan yang kita setujui
harus bisa dikutip saat review, bukan bergantung pada skill mana yang kebetulan aktif.

| ID | Aturan | Kenapa |
|---|---|---|
| **W-1** | **Namai sesuatu dengan apa yang dikendalikan pengguna, bukan bagaimana sistem dibangun.** Orang mengatur *notifikasi*, bukan *webhook config* | Kosakata antarmuka adalah papan penunjuk arah |
| **W-2** | **Kontrol menyebut persis apa yang terjadi saat ia dipakai.** *"Confirm and upload"*, bukan *"Submit"* | Suara aktif; spesifik mengalahkan pintar |
| **W-3** | **Sebuah aksi memakai nama yang sama di sepanjang alur.** Tombol *Publish* menghasilkan pesan *Published* | Konsistensi adalah cara orang belajar jalan |
| **W-4** | **Error tidak minta maaf dan tidak pernah kabur tentang apa yang terjadi.** Sebutkan yang salah dan cara memperbaikinya | Sudah jadi aturan §14.5; ini menyatakannya sebagai aturan penulisan |
| **W-5** | **Tiap elemen mengerjakan satu tugas.** Label melabeli, contoh mencontohkan; tidak ada yang diam-diam merangkap | |
| **W-6** | Sentence case, bukan Title Case | Gaya rumah. Menimpa aturan Chicago di `web-design-guidelines` |

> **Kenapa W-4 layak dicatat dua kali.** Skill itu menuliskan *"Errors don't apologize, and they are never vague about what happened. An empty screen is an invitation to act"* — dan project ini sampai ke aturan yang sama sendiri di §14.5, lewat sebuah layar 500 yang menyuruh orang menambahkan data lalu menghapus semua caranya. Dua jalan berbeda menuju satu tempat adalah bukti yang lebih baik daripada salah satunya sendirian.

#### Lantai kualitas — dipenuhi tanpa diumumkan

| ID | Aturan | Status |
|---|---|---|
| **Q-1** | Fokus papan ketik selalu terlihat; `:focus-visible`, tidak pernah `outline: none` tanpa pengganti | ✅ `globals.css` |
| **Q-2** | `prefers-reduced-motion` dihormati | ✅ Satu animasi ada (sapuan bar unggah indeterminate) dan ia dimatikan di bawah `prefers-reduced-motion` |
| **Q-3** | Kontras memenuhi WCAG AA | ✅ Ditegakkan `tests/test_design_tokens.py` (NFR-UX.3) |
| **Q-4** | **Emoji bukan ikon.** Emoji berubah bentuk antar platform dan dibacakan screen reader sebagai nama Unicode-nya | ✅ **Ditutup 2026-08-01 (D-037).** `📌` diganti SVG 12px yang sengaja **bukan** pushpin — ia menggambarkan apa yang pin lakukan (membekukan kolom di tepi), bukan metafora menempel. Satu-satunya ikon di produk ini, dan itu disengaja |
| **Q-5** | Target sentuh — **AA adalah 24×24 (SC 2.5.8), bukan 44×44** | ✅ **Ditutup 2026-08-01 (D-037), dan premisnya dikoreksi.** 44 adalah SC 2.5.5, yang **AAA**; aturan yang dipanen dari UUPM menyebut ambang AAA tanpa mengatakannya, dan tombol kita ± 30px **sudah lulus AA sebelum sesi ini**. Lantainya kini 32px, dan `@media (pointer: coarse)` menaikkannya ke 44px — tablet dapat targetnya, meja tidak kehilangan baris |
...
> ⚠️ **Alasan di atas diperiksa ulang 2026-08-01 dan ternyata hanya separuh (D-036). Keputusannya tetap; alasannya diperluas** (§0.3 aturan 2 — alasan berubah, keputusan tidak).
>
> Paragraf itu seluruhnya tentang pengguna **baru** dengan satu project auto-buat. Ia tidak menyentuh pengguna **kembali** yang punya banyak — dan itu justru orang yang galeri klaim layani. Alasan yang berlaku untuk keduanya adalah **ongkos klik**: galeri menambahkan **satu klik wajib** ke kasus yang paling sering terjadi (buka aplikasi, lanjutkan di tempat terakhir) demi mempercepat kasus yang jarang (ganti project), yang hari ini sudah berharga dua klik lewat picker dengan nilai sekarang terlihat — *recognition*, bukan *recall*. Itu rugi bersih untuk pengguna baru maupun lama.
>
> **Batas kejujuran klaim ini dinyatakan, bukan disembunyikan:** ia berlaku selama seseorang punya segelintir project. Tidak ada bukti apa pun tentang tiga puluh, di mana picker menjadi daftar gulir tanpa pencarian. **Ditinjau di Gerbang 4.**
>
> Diperiksa juga: **"nol project" adalah keadaan yang tidak bisa dicapai.** §15.2 membuat satu otomatis saat registrasi, dan tidak ada rute yang menghapus project. Merancang layar pertama untuk keadaan itu akan menjadi pekerjaan tanpa pemakai, yang §20 larang.
| **Halaman gagal dimuat** | **Ganti seluruh badan halaman**, jangan ditumpuk di atasnya. Sebutkan apa yang gagal dalam kalimat manusia — bukan status HTTP — dan selalu sediakan jalan maju (biasanya *Try again*). Lihat catatan di bawah tabel. |
| **URL tidak menunjuk apa pun (404)** | Aturan yang sama, dan ia **berlaku untuk halaman bawaan framework juga** — itu yang terlewat sampai 2026-08-01 (D-036). Pakai shell aplikasi, sebutkan bahwa tautannya mungkin usang atau datanya sudah dihapus, dan sediakan satu tujuan yang selalu ada. **Jangan menebak maksudnya** — *"mungkin maksudmu…"* dari sebuah path adalah tebakan meyakinkan yang P6 tolak, dan diantar dengan yakin ke dataset yang salah lebih buruk daripada diberi tahu bahwa yang ini tidak ada. |
...
> **Aturan error state ditambahkan 2026-07-31, dari sebuah layar sungguhan.** API mati; `/auth/me` menjawab 500; beranda merender ini:
>
...
>
> Tanpa zona drop, tanpa kartu sampel, tanpa menu akun — ketiganya bergantung pada identitas yang baru saja gagal diambil. **Layar itu menyuruh menambahkan data, lalu menghapus setiap cara untuk melakukannya, dan tidak menyisakan jalan keluar** — bahkan sign out pun tidak.
>
> **Aturan yang melarangnya sudah ada di tabel ini sejak 2026-07-29**, hanya ditulis untuk empty state: *"setiap artefak yang ditampilkan wajib bisa dibuka; daftar nama tanpa jalan masuk bukan orientasi, ia jalan buntu."* Tidak ada yang pernah menguji error state terhadapnya. Karena itu ia kini punya barisnya sendiri, dan dua konsekuensinya dinyatakan:
>
> | | |
> |---|---|
> | **Mengganti, bukan menumpuk** | Instruksi yang halamannya sendiri sudah lumpuhkan lebih buruk daripada tidak ada instruksi. Banner di atas badan halaman hanya benar ketika **aksi**-nya yang gagal dan halamannya masih bekerja — memuat dataset contoh, misalnya. |
> | **Status HTTP bukan pesan** | `500` tidak menyebut apa yang terjadi maupun apa yang bisa dilakukan, dan bagi kebanyakan orang ia bahkan bukan informasi — ia angka yang menyiratkan mereka merusak sesuatu. |
>
> Ini kelas cacat yang sama dengan **baris dataset yang tidak bisa diklik** di catatan sebelumnya, dan ditemukan dengan cara yang sama: bukan oleh test, melainkan dengan memandangi layarnya.
...
| ~~28~~ | ~~Peringatan deteksi skema berisiko (FR-C.6)~~ — dicabut 2026-07-31; dipikul `type_consistency_report` | — |
...
>
> **R-16 dan R-17 ditambahkan setelah review Gerbang 1 (2026-07-29), dan keduanya adalah risiko yang kita pilih untuk ditanggung, bukan yang kita mitigasi.** Itu perbedaan yang layak dijaga kejujurannya.
>
> R-17 punya properti yang tidak dimiliki risiko lain di tabel ini: **ia sudah terbukti sekali.** Review Gerbang 1 menemukan aplikasi mengembalikan 500 di setiap rute basis data ketika dijalankan lewat uvicorn — murni karena Windows, dan tak terlihat oleh 236 test hijau. Cacat setara yang hanya muncul di Linux tidak punya cara ditemukan dari sini.
>
> R-16 tidak punya sinyal peringatan dini sama sekali, dan itu sengaja ditulis begitu di kolomnya. Setiap risiko lain di tabel ini memberi tanda sebelum menagih; kegagalan disk tidak. Ini juga satu-satunya risiko yang mitigasinya memakan waktu **satu menit** (`gh repo create --private`, lalu push) — rasio biaya terhadap dampak yang paling timpang di seluruh dokumen.

> **Penutupan R-16 dan penurunan R-17 (2026-07-29) — digerakkan bukti, bukan rencana.** §0.3 melarang menurunkan level risiko karena sudah punya niat; keduanya bergerak di sini karena tindakannya sudah dilakukan dan hasilnya sudah terlihat.
>
> **R-16 ditutup**, dan penutupannya datang tepat waktu dengan cara yang tidak nyaman: pembukaan Fase 2 menemukan bahwa `backend/app/storage/` — object store beserta lapisan yang menegakkan INV-7 L3 — **tidak pernah masuk git sama sekali**, karena pola `.gitignore` yang dimaksudkan untuk direktori data runtime cocok di kedalaman mana pun. Tidak ada satu pun sinyal: `git status` bersih, 242 test hijau, coverage bahkan melaporkan persentasenya. Selama R-16 masih berlaku, itu dua file sumber tanpa salinan di mana pun. Perhatikan bentuknya: **kolom "sinyal peringatan dini" R-16 berbunyi "tidak ada", dan yang hampir terjadi memang tidak memberi satu pun.**
>
> **R-17 turun Tinggi → Sedang**, bukan ditutup. CI kini berjalan, dan **run pertamanya langsung menemukan cacat khusus Linux** — `asyncio.ProactorEventLoop` tidak ada di sana, sehingga mypy gagal sementara seluruh pemeriksaan lokal hijau. Itu tepat arah yang sebelumnya dinyatakan mustahil ditemukan dari sini, dan ia ketahuan dalam 20 detik. Yang menahannya tetap di Sedang: satu run hijau membuktikan satu commit, bukan sebuah kebiasaan, dan job `test` di Linux baru sekali menyentuh Postgres sungguhan. Struktur pencegahnya sudah ditambahkan — mypy dijalankan dari **sudut pandang platform yang berlawanan** di CI maupun di `scripts/check.sh`, sehingga arah yang tidak sedang kita tempati tetap diperiksa. Ditinjau ulang setelah CI hijau melintasi beberapa fase.
...
**D-037 · Sistem visual: nol warna aksen, dua tanah dari satu deklarasi** 🟢 Accepted · 2026-08-01
*Konteks:* Pemilik produk memakai produknya dan menjawab *"apa yang terasa belum matang?"* dengan **"kurang punya karakter dan minim dipertimbangkan."** Diperiksa terhadap §18, itu harfiah benar: **setiap keputusan desain yang pernah project ini catat adalah pengurangan** — D-032, FR-C.6, urutkan kolom, badge versi, null%, sidebar, Run Log, composer, pil privasi. Tidak ada satu pun keputusan positif tentang rupa. `system-ui` adalah typeface yang dipilih dengan cara tidak memilih, dan paletnya slate kebiruan yang bisa jadi milik alat developer mana pun. Ketelitian project ini seluruhnya tercurah ke kebenaran dan ke apa yang dibuang; sistem visualnya adalah sisa, ditambah default.
*Keputusan:* Arah **B · Bobot** — hierarki dibangun bobot dan ketebalan garis; **`--accent` dihapus seluruhnya** dan hanya dua rona tersisa, keduanya untuk pengecualian. Tanah **netral dingin** (hue ±222°) dipilih pemilik produk dari tiga spesimen yang dirender pada header dan grid yang sama, di dua tanah. Dua tanah lahir dari **satu deklarasi per token** lewat `light-dark()`.
*Kenapa tanpa aksen sama sekali:* `--accent` sedang mengerjakan **lima pekerjaan** — tautan, tipe logis, tombol primer, setiap border hover, dan cincin fokus. Warna yang menandai *segala yang bisa diklik* tidak menyisakan apa pun untuk dikatakan ketika sesuatu benar-benar salah. Dan anggaran warnanya akan **dibutuhkan Fase 3**: `stale` dan `cached` (UX-4, UX-5) masing-masing butuh makna visual, dan arah yang membelanjakan warnanya untuk afordansi tidak punya sisa.
*Penggantinya, dan apa yang masing-masing kodekan:* tautan → garis bawah (satu-satunya sinyal web yang tak pernah ambigu); tombol primer → **dibalik**, isi `--ink` teks `--bg` (penekanan maksimum pada saturasi nol — arti harfiah "Bobot"); tipe kolom → muka mono + `--ink-faint` (mesin yang menyimpulkannya); cincin fokus → `--ink` (satu-satunya warna yang dijamin kontras di dua tanah tanpa perlu dibuktikan ulang per permukaan); hover → `--ink-dim`. Pilihan "sedang aktif" — tipe terpilih, kolom ter-pin, tanah terpilih — semuanya memakai **pembalikan yang sama**, jadi satu gerakan dipelajari sekali.
*`--ok` ikut dicabut, dan itu bukan efek samping:* sukses bukan pengecualian, ia hasil yang normal. *"Password changed. 3 other sessions were signed out."* sudah menyatakan segalanya; hijau tidak menambah satu fakta pun, jadi menurut aturan review sesi ini ia **dekorasi**.
*Kenapa `light-dark()` dan bukan dua blok `:root`:* dua blok yang harus sepakat adalah dua blok yang suatu hari tidak lagi sepakat, dan project ini sudah menghabiskan beberapa sesi untuk hal-hal yang menyimpang karena tidak ada yang membandingkannya. Satu deklarasi menaruh kedua nilai **di baris yang sama**, sehingga `test_design_tokens.py` membacanya sekaligus dan drift menjadi mustahil secara konstruksi. `color-scheme` juga membawa serta widget native — scrollbar, checkbox, select, autofill — yang tidak mengikuti CSS kita. Dukungan: Chrome 123, Safari 17.5, Firefox 120.
*Apa yang ditemukan dengan menulis test-nya lebih dulu (langkah 6 brief):* menahan **satu** warna border pada 3:1 (WCAG 1.4.11) akan menggambar ribuan pemisah baris grid dengan bobot yang sama seperti outline tombol. Maka border pecah jadi **tiga token dengan tiga pekerjaan** — `--edge` (ini kontrol; ditahan pada 3:1 dan diuji), `--line` (batas antar-region), `--line-soft` (baris dari hal yang sama). **Token itu tidak ada di rencana; test-lah yang melahirkannya.** Urutan "test dulu, CSS belakangan" terbukti bukan formalitas.
*Q-4 ditutup:* `📌` diganti SVG 12px, dan **sengaja bukan gambar pushpin** — pushpin adalah metafora *menempel*, sementara yang terjadi adalah membekukan kolom di tepi sementara sisanya bergeser lewat. Bentuknya batang di tepi dengan garis-garis menjauh darinya. Ia **satu-satunya ikon di produk ini**, dan jawaban atas *"apakah produk ini punya ikonografi?"* adalah **hampir tidak, dan sebaiknya tetap begitu**: satu ikon untuk satu kontrol yang tidak mampu memuat kata di dalam daftar 60 baris.
*Q-5 — premisnya keliru, dan koreksinya terukur:* ambang **WCAG 2.2 AA (SC 2.5.8) adalah 24×24, bukan 44×44**. Angka 44 adalah SC 2.5.5, yang **AAA**, dan pedoman Apple. Tombol kita ± 30px, jadi ia **sudah lulus AA** sebelum sesi ini — aturan yang dipanen dari UUPM menyebut ambang AAA tanpa mengatakannya. Lantainya kini 32px, dan `@media (pointer: coarse)` menaikkannya ke 44px, sehingga tablet mendapat targetnya tanpa meja kehilangan satu baris pun. Pertukaran kepadatan diselesaikan **per perangkat masukan**, bukan dengan memilih salah satu.
*Typeface — `system-ui` dipertahankan, dan itu kini keputusan:* muka yang di-host sendiri menambah byte ke setiap muat halaman terhadap anggaran NFR-PERF.1; permukaan ini ditatap berjam-jam dan muka sistem adalah yang sudah di-hint untuk layar orang itu sendiri; dan **membeli karakter dari muka display berarti membelanjakannya persis di tempat yang arah B larang.** ⚠️ Dinyatakan apa adanya: **hasilnya sama dengan default**, dan yang berubah hanyalah bahwa ia punya alasan dan syarat peninjauan.
*Tema adalah tiga keadaan, bukan dua:* "ikuti mesinku" adalah jawaban nyata dan default-nya. Sakelar dua posisi tidak bisa menyatakannya — klik pertama diam-diam mengubah preferensi yang *mengikuti* menjadi preferensi yang *terpaku*, tanpa jalan kembali. Kontrolnya di menu akun: satu baris di dalam launcher yang sudah ada, bukan region baru di shell (aturan ② sesi ini).
*Alternatif yang ditolak:* **(a)** netral hangat/kertas — ditolak pemilik produk setelah melihat spesimennya; **(b)** netral hijau-kelabu — sama, meski ia yang paling jarang dipakai alat teknis; **(c)** mempertahankan aksen hanya untuk tautan — setengah aturan adalah aturan yang akan diperdebatkan tiap kali, dan lima pekerjaan aksen itu semuanya masuk akal satu per satu; **(d)** muka display — lihat di atas; **(e)** dua blok `:root` — lihat di atas.
*Trade-off yang dinyatakan, bukan disembunyikan:* **netral dingin adalah seragam hampir setiap alat teknis**, dan brief sesi ini memperingatkannya di muka. Memilihnya berarti menerima terbaca sebagai salah satu dari banyak — sehingga **nol karakter boleh datang dari palet**, dan seluruh bebannya pindah ke bobot, tiga tier garis, dan ritme. B yang dikerjakan malas menjadi persis keluhan yang memulai sesi ini, hanya lebih kelabu. Selain itu: skala spasi 4px **generik dan diakui generik** — ritme bukan tempat karakter seharusnya tinggal.
*Ditinjau ulang bila:* Fase 3 tiba dan `stale`/`cached` menuntut lebih dari dua rona (di situlah anggaran yang dihemat ini dibelanjakan); atau produk ini punya permukaan publik, yang mengubah perhitungan typeface; atau penguji Gerbang 4 tidak menemukan afordansi tanpa warna — yang akan menjadi bukti terhadap §4.3, bukan alasan menambal satu layar.

---

**D-036 · Orientasi hidup di baris header yang sudah ada, dan versi menyebut project-nya** 🟢 Accepted · 2026-08-01
*Konteks:* Pemilik produk memakai produknya dan menyimpulkan navigasinya *"masih nol dan berantakan"*. Diperiksa dengan menghitung rute — bukan mengingat — aplikasi ini punya **dua tujuan kerja** (`/` dan `/versions/[id]`), satu tujuan akun yang sudah terjangkau dari mana saja, dan sebuah 404. Di antara keduanya ditemukan sesuatu yang lebih berat daripada keluhan aslinya: `ProjectPicker` hanya dirender di `/`, sehingga satu-satunya layar yang **tidak bisa** menjawab *"aku di project mana"* adalah layar yang jawabannya tidak sudah tertulis di badan halaman. Dan satu-satunya jalan keluar di dalam aplikasi dari halaman versi adalah tautan brand, yang menuju `/` — tempat beranda membuka project mana pun yang terakhir diingat `localStorage`. Membuka versi milik project A dari bookmark saat browser mengingat project B, lalu menekan brand, **memindahkan pengguna ke project B tanpa satu pun tanda di layar.** Itu kesalahan orientasi, bukan afordansi yang hilang.
*Keputusan:* **(1)** Jejak `Project › Dataset` masuk ke slot `headerExtras` yang sudah dimiliki `Shell` dan hanya dipakai beranda — bukan region baru, bukan baris baru. **(2)** `DatasetVersionResponse` membawa `project_id` dan `project_name`. **(3)** `app/not-found.tsx` menggantikan bawaan Next. **(4)** Nama project dan caret picker adalah **dua kontrol**, bukan satu chip. **(5)** Skip link ke `<main>`.
*Kenapa ini tidak menggugat D-032:* argumen pencabutan sidebar adalah **ongkos lebar** — 190px dari setiap layar 1280px untuk satu tautan duplikat dan satu tautan rusak. Jejak ini memakai **nol lebar tambahan dan nol tinggi tambahan**; ia mengisi slot yang sudah ada. Hitungan "dua tujuan" yang mendasari D-032 diperiksa ulang di sesi ini dan **masih benar**, jadi sidebar tetap dicabut dan syarat kembalinya (`Library` & `Steps`, Fase 3) tidak berubah. Usulan mengembalikannya **ditolak secara eksplisit**, meski brief sesi ini membuka pintunya.
*Kenapa perubahan backend, bukan kecerdikan klien:* respons versi membawa `dataset_id` dan `dataset_name` dan **tidak ada apa pun tentang project**, jadi halaman itu tak punya bahan untuk menyebut maupun menautkan project-nya. Nol migrasi — Dataset→Project sudah foreign key; ini fakta yang sudah ada, dinyatakan. Presedennya berumur satu hari: `dataset_name` ditambahkan ke respons yang sama, atas alasan yang sama.
*Kenapa nama dan caret dipisah:* sebagai satu chip picker, meninggalkan halaman versi berharga **dua klik** — buka menu, pilih project yang sedang kamu tempati. **Jalan keluar tidak boleh lebih mahal daripada perpindahan yang lebih jarang di sebelahnya.** Dipisah: keluar satu klik, ganti project dua. Ongkosnya satu tab stop, dan ia di header — bukan di daftar 60 kolom, jadi aritmetika yang mencabut tombol urut (D-032) dan gagang resize yang bisa difokus (FR-D.3) tidak menjangkaunya.
*Alternatif yang ditolak:* **(a)** galeri project sebagai layar pertama — lihat koreksi §14.5 di bawah; **(b)** sidebar kiri kembali — hitungannya menolak; **(c)** baris kedua khusus breadcrumb — ± 32px tinggi di setiap layar untuk sesuatu yang muat di slot yang sudah kosong, dan ia **region baru**, yang justru dilarang aturan sesi ini; **(d)** brand menjadi tombol *back* — kontrol yang kadang pulang dan kadang mundur tidak bisa diprediksi dan menyembunyikan tujuannya (Nielsen #3 meminta jalan keluar yang **jelas**, bukan yang pintar).
*Segmen ketiga sengaja belum ada.* Jejak penuhnya `Project › Dataset › Versi`, tapi hari ini satu Dataset selalu punya tepat satu versi terjangkau — tidak ada rute yang mendaftar versi — sehingga *Dataset* dan *Versi* adalah satu hal. Menggambarnya sebagai dua berarti merender hierarki yang belum bisa dijelajahi. Segmen ketiga datang bersama **FR-B.2 + P4**, yang sudah dijadwalkan sebagai satu pekerjaan.
*Trade-off:* halaman versi kini melakukan **satu request tambahan** (`api.projects`) untuk mengisi picker, pada halaman yang terukur 94 ms terhadap anggaran NFR-PERF.1 1.000 ms. Dan `h1` di kedua layar kerja kini mengulang segmen terakhir jejak — nyata, ringan, dan diserahkan ke pekerjaan ritme badan halaman.
*Konsekuensi:* `localStorage` project berpindah dari konstanta di dalam `page.tsx` ke `lib/lastProject.ts`, karena halaman versi kini juga menulisnya; menulis dari satu tempat saja akan membuat brand mengembalikan pengguna ke project yang baru saja ditinggalkan.
*Ditinjau ulang bila:* tujuan kerja bertambah melewati dua (Fase 3 — `Library`, `Steps`, tab Profile), atau seseorang benar-benar punya cukup banyak project sehingga picker tanpa pencarian menjadi beban (**Gerbang 4**). Klaim "galeri rugi bersih" di §14.5 berlaku untuk segelintir project dan **tidak diuji pada tiga puluh**.

---

**D-035 · Skill pihak ketiga di-vendor ke repo, dan tunduk pada DESIGN.md** 🟢 Accepted · 2026-08-01
*Konteks:* Project ini tidak punya desainer, dan pekerjaan frontend sejauh ini **memperbaiki cacat, bukan mendesain**. Lima paket panduan agen dievaluasi: `frontend-design` (the vendor), `web-design-guidelines` · `react-best-practices` · `composition-patterns` (Vercel), dan UI/UX Pro Max (Next Level Builder). Kelimanya dibaca utuh sebelum keputusan; **tiga dari lima ternyata berbeda dari dugaan yang dibuat berdasarkan nama repo saja**, yang cukup untuk menjadikan "baca dulu, pasang belakangan" sebagai aturan dan bukan formalitas.
*Keputusan:* Tiga skill Vercel di-vendor ke `config/skills/` **di dalam repo**, bukan dipasang di `~/config/`. Dua sisanya tidak dipasang; aturan yang berguna darinya diangkat ke §14.4 sebagai aturan kita sendiri.
*Kenapa di repo:* skill adalah **instruksi yang membentuk cara kerja di sini**, bukan alat pribadi. Di `~/config/` ia berbeda antar mesin, tak terlihat di CI, dan berubah tanpa jejak — tiga sifat yang project ini tolak untuk semua hal lain yang mempengaruhi hasil (opsi tulis Parquet dipin, fixture ber-SHA-256, fingerprint dari input berversi). Biayanya: ± 500 KB teks pihak ketiga di repo, dan tanggung jawab meninjau diff-nya saat diperbarui.
*Aturan presedensi — bagian terpenting keputusan ini:* **DESIGN.md mengalahkan setiap skill.** Temuan adalah masukan, bukan vonis. Skill yang meyakinkan kita bahwa sebuah keputusan keliru menghasilkan **ADR baru dengan alasannya**, bukan suntingan diam-diam. Tanpa aturan ini, panduan generik akan menggerus posisi yang dibayar mahal: hampir semua panduan frontend mendorong component library, hierarki visual lebih ramai, dan motion — sementara §10.3 menolak component library, P9 memilih permukaan kecil, dan D-032 baru saja mencabut sepertiga lebar layar. Dua konflik nyata sudah muncul dan keduanya diselesaikan untuk DESIGN.md: **Title Case** (kita sentence case) dan **curly quotes** (kita lurus).
*Satu modifikasi lokal:* `web-design-guidelines` aslinya mengambil aturannya dari GitHub **setiap kali review**. Diubah agar membaca salinan ter-commit. Kumpulan aturan yang berubah antar-run bukan kumpulan aturan — dua run berbeda pendapat dan tak satu pun salah — dan review tidak boleh berjarak satu kegagalan jaringan dari diam-diam tidak memeriksa apa pun.
*Alternatif yang ditolak:* **(a)** pasang kelimanya — tiga di antaranya panduan desain yang tumpang tindih tapi tidak identik, jadi konfliknya harus diselesaikan di tengah pekerjaan, dan **cara penyelesaiannya tidak akan tercatat di mana pun**; **(b)** pasang UI/UX Pro Max — bukan satu skill melainkan **tujuh** (23 MB), lima di antaranya untuk logo/banner/slide/brand, dengan **33 skrip yang dapat dieksekusi** (sepuluh menyentuh jaringan, subprocess, atau menulis berkas) termasuk `shadcn_add.py` yang memasang persis component library yang §10.3 tolak. Bagian bergunanya — checklist UX-nya — sebagian besar tumpang tindih dengan daftar Vercel yang 4 KB dan tanpa skrip; **(c)** pasang `frontend-design` — kerangkanya identitas visual (hero sebagai tesis, pasangan typeface, *"ambil satu risiko estetis"*), bukan alat analisis internal. Sepertiga terakhirnya dipanen; skill itu sendiri berkata *"the brief's own words always win"*, sehingga instruksinya sendiri sejalan dengan aturan presedensi di atas.
*Bukti bahwa ini bukan teater — dua temuan di hari pertama, keduanya di kode yang ditulis beberapa jam sebelumnya:* `📌` dipakai sebagai ikon tombol pin (UUPM prioritas 4 — emoji berubah bentuk antar platform dan dibacakan sebagai nama Unicode-nya), dan `gridPreferences.ts` menulis `localStorage` **tanpa versi skema** (`client-localstorage-schema`). Yang kedua diperbaiki bersama keputusan ini; yang pertama menuntut memilih pengganti visualnya, jadi ia dicatat sebagai Q-4 di §14.4 dan menunggu sesi desain.
*Amandemen 2026-08-01 — `frontend-design` akhirnya dipasang, dan alasan penolakannya di atas terlalu lebar.* Yang mengungkapnya adalah jawaban pemilik produk atas *"apa yang terasa belum matang?"* — **"kurang punya karakter dan minim dipertimbangkan."** Diperiksa terhadap §18, itu **harfiah benar**: setiap keputusan desain yang pernah project ini catat adalah **pengurangan** (D-032 · FR-C.6 · urutkan kolom · badge versi · null% · sidebar · Run Log · composer · pil privasi). **Tidak ada satu pun keputusan positif tentang rupa** — `system-ui` adalah typeface yang dipilih dengan cara tidak memilih, dan paletnya bisa jadi milik alat developer mana pun. Ketelitian project ini seluruhnya tercurah ke kebenaran dan ke apa yang dibuang; sistem visualnya adalah sisa, ditambah default. Itu persis celah yang **metode** skill ini isi: susun token ringkas, lalu uji rencananya dengan *"apakah ini yang akan kuhasilkan untuk brief serupa mana pun?"* ⚠️ **Metodenya diambil, kerangkanya tidak** — "hero sebagai tesis" dan "ambil satu risiko estetis" ditulis untuk landing page, bukan permukaan kerja padat yang ditatap berjam-jam. Aturan presedensi di atas tetap berlaku atas setiap katanya. Catatan lisensi: skill ini justru kasus yang **lebih kuat** dari ketiga skill Vercel — ia membawa `LICENSE.txt` sendiri (Apache 2.0), ikut di-vendor.

*Trade-off:* teks pihak ketiga kini ada di dalam repo, dengan lisensi MIT yang **hanya dinyatakan di frontmatter tiap skill** — repo hulunya tidak punya berkas `LICENSE` di root pada commit yang di-vendor. Dicatat apa adanya di `config/skills/README.md` alih-alih dihaluskan, karena "MIT menurut frontmatter" dan "MIT menurut LICENSE" bukan bukti yang sama.
*Ditinjau ulang bila:* sebuah skill berulang kali menghasilkan temuan yang kita tolak (tandanya salah alat, bukan salah kode), atau saat pembaruan hulu tiba dengan diff yang tidak lagi kita setujui.

---

**D-034 · Unggah dilaporkan progresnya lewat XHR, dan berhenti melapor saat tidak lagi tahu** 🟢 Accepted · 2026-07-31
*Konteks:* NFR-UX.2 mewajibkan indikator progres untuk setiap operasi di atas 500 ms. Gerbang 2 mengukur unggahan 480 MB pada **28 detik** — 56× ambangnya — dan yang ditampilkan hanyalah label tombol `Uploading…` yang tidak pernah berubah.
*Keputusan:* Satu jalur `XMLHttpRequest` khusus untuk `createDataset`; sisa `api.ts` tetap `fetch`.
*Kenapa bukan `fetch`:* ia **tidak bisa** melaporkan progres unggah. Body request hanya dapat diamati dengan menstreamnya, yang menuntut HTTP/2 + dukungan duplex yang belum bisa diandalkan. `XMLHttpRequest` punya `upload.onprogress` sejak lama. Ini batas kemampuan, bukan preferensi gaya — dan itulah alasan pengecualian satu ini dapat dipertahankan.
*Bagian yang lebih penting: indikatornya berhenti mengklaim angka.* Progres hanya mencakup **byte yang meninggalkan browser**. Setelah byte terakhir terkirim, server masih menormalkan ke Parquet dan menginferensi tipe dengan memindai **seluruh** baris (FR-B.3) — pada fixture 480 MB, itu bagian terbesar dari 28 detik dan sama sekali tidak terlihat dari klien. Karena itu pada 100% bar-nya menjadi *indeterminate* dan teksnya menyebut fase yang sedang berjalan.
*Alternatif yang ditolak:* **(a)** biarkan bar merayap ke 99% lalu menunggu — ia terbaca "hampir selesai" padahal kenyataannya "kira-kira separuh, dan separuh yang tak terlihat justru yang lambat". Sebuah indikator yang **menyatakan keadaan yang tidak diamatinya** adalah kelas kesalahan yang sama dengan pil `🔒 balanced` yang dicabut D-032, hanya lebih halus; **(b)** endpoint polling status server — model job asinkron untuk satu layar, jauh melebihi masalahnya, dan menambah permukaan yang harus diamankan; **(c)** pindahkan seluruh `api.ts` ke XHR demi konsistensi — menukar API modern untuk sebelas panggilan demi satu yang butuh.
*Trade-off:* `api.ts` kini punya dua mekanisme transport, dan penanganan error XHR harus **menyalin** semantik `request()` (detail dari body JSON, `ApiError` dengan status). Duplikasi itu nyata dan disengaja; status `0` dipakai untuk "tidak sampai ke server" dan tidak pernah diperlakukan sebagai status HTTP.
*Ditinjau ulang bila:* request body streaming bisa diandalkan lintas browser target, atau ingest berpindah ke model job asinkron (mis. saat NFR-SCALE.4 naik jauh di atas 500 MB).

**D-033 · Frontend punya test perilaku, dan `tsc` bukan penggantinya** 🟢 Accepted · 2026-07-31
*Konteks:* Sampai keputusan ini, frontend hanya punya dua pemeriksaan: `tsc --noEmit` dan `next build`. Keduanya membuktikan kode itu **valid**, tidak satu pun membuktikan ia **benar**. Yang memicu: `PreviewGrid` mengambil sel dengan indeks kolom **yang terlihat**, sementara `GET .../rows` mengirim satu sel per kolom **di file**. Menyembunyikan kolom mana pun selain yang terakhir menggeser setiap kolom di kanannya ke nilai tetangganya — header benar di atas nilai salah, tanpa error, tanpa sel kosong. Kedua sisi bertipe `(string | null)[]`, jadi `tsc` puas dan build sukses.
*Keputusan:* Pasang **vitest + Testing Library + jsdom**, jalankan di `scripts/check.sh` dan sebagai satu step di job `frontend` CI.
*Kenapa sekarang, bukan Fase 3:* Fase 3 menambah Run Log, panel Step, dan tab Profile — tiga permukaan baru dengan kelas cacat yang sama. Memasang alatnya nanti berharga sama, tapi dengan tiga komponen yang sudah terlanjur dibangun tanpa test.
*Ia tidak menggantikan `tests/test_frontend_wiring.py`.* Test itu menjawab pertanyaan yang tidak bisa dijawab renderer: *apakah ada yang dibangun tapi tak terjangkau dari UI*. Ia membaca `api.ts` secara statis. Keduanya tidak beririsan.
*Keputusan turunan yang layak dicatat:* **(a)** `globals: false` — `describe`/`it`/`expect` diimpor, karena `tsconfig` mencakup file test dan ruang tipe global sebaiknya tetap jujur. **(b)** Tanpa `@vitejs/plugin-react`: ia untuk Fast Refresh dan transform Babel, keduanya tak dipakai jsdom, dan `esbuild.jsx: "automatic"` sudah menutup satu-satunya kebutuhan nyata. Ia juga **tidak bisa dipasang dengan jujur** — plugin versi kini menuntut Vite 8 sementara Vitest 3 membawa Vite 7, sehingga pohon dependensi memuat dua salinan dan `tsc` menolak tipe plugin terhadap tipe config yang menerimanya. **(c)** `pool: "threads"`, bukan default `forks`: di Windows pool fork memutus kanal IPC-nya begitu tiga berkas test berjalan bersamaan (`ERR_IPC_CHANNEL_CLOSED`) sementara tiap berkas lulus sendirian. **Suite yang gagal hanya ketika utuh lebih buruk daripada suite yang lambat**, karena ia mengajari orang menjalankan ulang sampai hijau.
*Alternatif yang ditolak:* **(a)** perbaiki bug-nya secara struktural saja lalu catat ketiadaan test sebagai utang tertulis — jujur dan murah, tapi cacat UI berikutnya tetap tak ada yang menangkap; **(b)** Playwright/end-to-end — menangkap lebih banyak dan menuntut server, database, dan browser untuk menjawab pertanyaan sesempit "apakah sel ini milik kolom itu"; layak dipertimbangkan lagi di Gerbang 4, bukan di sini; **(c)** Jest — Vitest memakai konfigurasi Vite/esbuild yang sudah dipahami toolchain ini.
*Trade-off:* enam devDependency baru dan satu step CI. Ditambah satu kewajiban yang mudah dilupakan: **file test ikut diperiksa `tsc` strict**, jadi fixture yang tidak lengkap gagal sebagai error tipe — sudah terjadi sekali, dan itu perilaku yang diinginkan.
*Ditinjau ulang bila:* suite ini tumbuh melampaui apa yang wajar dijalankan di setiap commit, atau Gerbang 4 menuntut cakupan end-to-end yang sesungguhnya.

---

**D-032 · Permukaan kosong dicabut, bukan disimpan sebagai kerangka** 🟢 Accepted · 2026-07-31
*Konteks:* Fase 2 menampilkan seluruh bentuk §14.2 lebih dulu — sidebar navigasi, panel Run Log, composer copilot, indikator mode privasi — semuanya kosong dan masing-masing menyebut fase asalnya. Alasannya §14.1: pengguna harus menangkap model mentalnya dalam tiga puluh detik, dan tata letak yang menumbuhkan region baru mengajarkannya dua kali.
*Keputusan:* **Cabut semuanya sampai ada isinya**, dan catat syarat kembalinya di the project notes.
*Yang menjatuhkannya adalah angka, bukan selera:* sidebar 190px + Run Log 260px + strip composer ≈ **sepertiga lebar layar 1280px** yang dijamin NFR-UX.4 — dipakai untuk ruang yang tidak bisa disentuh, sementara yang terdesak adalah grid pratinjau, satu-satunya permukaan yang bekerja (FR-D.1).
*Alasannya juga lebih lemah dari yang tertulis.* "Ajarkan tata letak sekali" berlaku untuk **region**; kolom chat adalah kontrol yang sudah dikenal semua orang dan tidak mengajarkan apa pun dengan muncul lebih awal. Sidebar bahkan belum menjadi navigasi: satu tautan duplikat (`Preview` → `/`, sama dengan tombol brand), satu tautan rusak (`Schema` → `href="#"`, dirender seperti item aktif), dan tiga label fase.
*Satu di antaranya kelas berbeda:* pil `🔒 balanced` **menyatakan sebuah keadaan**, bukan mengumumkan ketiadaan. Gembok + nama mode terbaca "perlindungan aktif" apa pun tooltip-nya, dan tidak ada egress sama sekali hari ini — justru itu bahayanya: kebiasaannya bertahan sampai hari copilot mendarat, saat labelnya menjadi kebohongan tentang sistem yang hidup.
*Alternatif yang ditolak:* **(a)** pertahankan semua demi konsistensi visual — membayar ongkos ruang setiap hari selama berbulan-bulan demi manfaat satu kali; **(b)** isi dengan contoh — baris palsu di Run Log menyiratkan traceability yang belum ada, dan P3 adalah janji yang tidak boleh diperlakukan santai; **(c)** sambungkan pil privasi ke kolom aslinya sekarang — jujur soal *nilai*, masih menyesatkan soal *keberlakuan*.
*Trade-off:* Fase 3 dan 5 akan menumbuhkan region baru, dan pengguna saat itu bertemu tata letak yang berubah. Diterima: itu satu pelajaran nanti, versus sepertiga layar hari ini.
*Ditinjau ulang bila:* masing-masing punya isi — Run Log & sidebar di Fase 3, composer & indikator privasi di Fase 5. **Indikator privasi hanya boleh kembali bersama ketiganya sekaligus:** rute yang mengembalikan `workspace_policy.llm_privacy_mode`, Privacy Gate yang menegakkannya (§13.5), dan kontrol untuk menggantinya.

**D-031 · Akun punya halamannya sendiri; dropdown hanya peluncur** 🟢 Accepted · 2026-07-31
*Konteks:* FR-A.2 (*"sesi pengguna dapat dicabut: logout, logout semua perangkat"*, **P0**) diimplementasikan penuh di backend sejak Fase 1 — satu baris `Session` per perangkat, token di-hash SHA-256, dua rute hidup, dua test integrasi — lalu **tidak pernah diberi tombol**. Satu-satunya cara keluar adalah menghapus cookie lewat DevTools. Sebuah requirement P0 tidak terpenuhi di permukaan produk sementara setiap test per-lapis hijau, karena lubangnya berada **di antara** lapisan.
*Keputusan:* **Dropdown + halaman, dengan pembagian tugas tegas.** Dropdown memuat identitas dan satu aksi yang sering dipakai (`Sign out`); apa pun yang butuh form, daftar, atau kalimat penjelasan pindah ke `/settings/account`. Popup yang harus digulir adalah bentuk yang salah untuk isinya.
*Isi halaman, dan dasarnya:* identitas · ganti password · **daftar sesi aktif dengan pencabutan per-perangkat**. Yang ketiga mengikuti OWASP Session Management Cheat Sheet: pengguna harus bisa memeriksa sesi hidupnya (alamat, klien, waktu mulai, terakhir aktif) dan mencabutnya dari jarak jauh. **Seluruh kolom yang dibutuhkan sudah ada di `user_session` sejak Fase 1** — yang hilang hanya query dan halamannya, nol migrasi.
*Ganti password menuntut password lama* meski pemanggilnya sudah terautentikasi. Tampak berlebihan; tidak: itulah yang mencegah laptop terbuka yang dipinjam menjadi pengambilalihan permanen. Ia **mencabut setiap sesi lain** (OWASP menyebutnya *security boundary event*) tapi mempertahankan sesi pemanggil — mengeluarkan seseorang dari tab yang sedang ia pakai terbaca sebagai kegagalan. Ia juga **mematikan token reset yang masih beredar**: tanpa itu ada lubang nyata — penyerang meminta reset, korban merasa ada yang salah lalu mengganti password, dan token di email penyerang tetap sakti.
*Email ditaruh di tombolnya, bukan hanya di dalam menu.* Pola lazim adalah lingkaran avatar, tapi pola itu mengandaikan satu akun per orang. Pertanyaan *"aku sedang pakai akun apa?"* harus terjawab tanpa klik, karena mengunggah ke akun yang salah adalah kesalahan nyata dengan biaya pembersihan nyata.
*Alternatif yang ditolak:* **(a)** jejalkan semuanya ke dropdown — daftar sesi memaksa popup yang harus digulir; **(b)** halaman Settings penuh dengan billing, MFA/SSO, anggota, hapus akun — billing disebut §19 sebagai contoh baku scope creep (R-3), MFA/SSO adalah FR-A.7 prioritas P3, anggota adalah permukaan Workspace yang justru sengaja tidak disebut di UI, dan hapus akun tidak ada di satu pun FR; **(c)** hanya `Sign out` tanpa daftar perangkat — menyisakan pilihan "cabut yang ini" atau "cabut semua", tanpa pernah bisa "cabut yang itu", yaitu justru kasus laptop tertinggal di kantor.
*Trade-off:* dua permukaan untuk dirawat alih-alih satu, dan satu endpoint backend baru (`POST /auth/password`) yang tidak menutup lubang mana pun — FR-A.1 hanya menyebut daftar dan masuk. Diterima: halaman akun tanpa cara mengganti password terasa setengah jadi bagi siapa pun yang membukanya.
*Cacat yang ditemukan riset terhadap versi pertama dropdown ini, keduanya buatan sendiri:* **(1)** ia mendeklarasikan `role="menu"`/`menuitem` — peran yang **menjanjikan** navigasi panah, Home/End, typeahead huruf pertama, dan manajemen fokus komposit — sementara tidak satu pun diimplementasikan. Mengumumkan kontrak ke teknologi bantu lalu melanggarnya lebih buruk daripada diam; diganti **pola disclosure** (`aria-expanded` + elemen native) sesuai WAI-ARIA APG dan MDN. **(2)** ia tidak bisa ditutup dari keyboard sama sekali — Tab berjalan melewati panel ke halaman di belakangnya sementara panelnya menggantung terbuka. **(3)** `Sign out on all devices` tayang tanpa konfirmasi, satu baris di bawah item yang bunyinya nyaris sama — **melanggar NFR-UX.1/UX-7**, aturan kita sendiri, dan keputusan itu diambil secara sadar tanpa mengecek NFR-nya.
*Ditinjau ulang bila:* undangan antar-akun mendarat (nama workspace dan peran jadi pembeda dan pantas tampil lagi), atau MFA/SSO dipicu §16.
...
| ~~**R-16**~~ | ~~Seluruh project hanya ada di satu laptop~~ → **✅ Ditutup 2026-07-29.** Remote privat `nichodarren/datacanvas` dipasang, seluruh riwayat ter-push | Eksekusi | — | — | **Nyaris menagih sebelum ditutup:** seluruh paket `backend/app/storage/` ternyata tak pernah masuk git karena pola `.gitignore` yang cocok di kedalaman mana pun. `git status` bersih, 242 test hijau, coverage menghitungnya. Dijaga `tests/test_repo_hygiene.py` | — |
| **R-17** | **CI baru dieksekusi sejak 2026-07-29** — perilaku di Linux belum lama terverifikasi | Teknis | ~~Tinggi~~ **Sedang** | Sedang | `scripts/check.sh` menjalankan seluruh rangkaian di runner mana pun; `.gitattributes` memaku line ending; **`--platform` lintas-platform kini dijalankan di CI *dan* di `check.sh`** | **Sudah terjadi dua kali, satu ke tiap arah.** (1) Review Gerbang 1: aplikasi tidak jalan di bawah uvicorn — cacat khusus Windows yang lolos 236 test. (2) **Run CI pertama:** `asyncio.ProactorEventLoop` tidak ada di Linux, jadi mypy gagal di sana sementara seluruh pemeriksaan lokal hijau — persis arah yang kolom ini sebut "tidak akan ketahuan sampai deploy". Ia ketahuan dalam 20 detik |
